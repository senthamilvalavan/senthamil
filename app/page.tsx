  ...
  >
    <div className="flex flex-col items-start mt-8 md:mt-0">
      <motion.h1 className="text-4xl md:text-5xl lg:text-[3.5rem] xl:text-6xl font-bold tracking-tighter mb-4 flex flex-col">
        <motion.span variants={springUp} className="text-xl md:text-3xl text-white/70 mb-2 font-medium tracking-normal">
          Hi, I&apos;m
        </motion.span>

        {/* Bungee Shade Font Applied Here - Keeping your specific gaps & sizes with mobile support */}
        <div className={`flex flex-wrap gap-x-4.5 gap-y-2.5 overflow-visible text-[2rem] sm:text-[2.5rem] md:text-[4rem] lg:text-[5.3rem] leading-[1] ${bungeeShade.className}`}>
      ...
      <motion.h2 variants={springUp} className="text-lg md:text-2xl text-white/80 mb-6 font-light">
        Data Scientist & Data Analyst
      </motion.h2>

      <motion.p variants={springUp} className="text-gray-500 mb-8 md:mb-10 leading-relaxed max-w-md text-base md:text-lg">
        Results-driven Data Scientist turning raw datasets into actionable insights through Explainable AI (XAI), advanced SQL querying, and scalable ML pipelines.
      </motion.p>

      <motion.div variants={springUp} className="flex flex-wrap gap-3 md:gap-4">
        <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} href="#contact" className="px-6 md:px-8 py-3 bg-white text-black rounded-full font-medium shadow-[0_0_20px_rgba(255,255,255,0.1)] text-center text-sm md:text-base">
          Hire Me
        </motion.a>
        <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} href="/Senthamil_Valavan_S_DATA_SCIENCE.pdf" download="Senthamil_Data_Science_Resume.pdf" className="px-5 md:px-6 py-3 border border-white/20 text-white rounded-full font-medium hover:bg-white/10 transition-colors flex items-center gap-2 text-sm md:text-base">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>
          DS Resume
        </motion.a>
        <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} href="/Senthamil_Valavan_S.pdf" download="Senthamil_Data_Analyst_Resume.pdf" className="px-5 md:px-6 py-3 border border-white/20 text-white rounded-full font-medium hover:bg-white/10 transition-colors flex items-center gap-2 text-sm md:text-base">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>
          DA Resume
        </motion.a>
        <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} href="#projects" className="px-5 md:px-6 py-3 border border-white/5 text-gray-400 rounded-full font-medium hover:text-white transition-colors text-sm md:text-base">
          View Work
        </motion.a>
      </motion.div>
    </div>

    <motion.div variants={springUp} className="flex justify-center md:justify-end relative mt-10 md:mt-0">
      <motion.div
        animate={{ y: [0, -15, 0] }}
        transition={{ repeat: Infinity, duration: 5, ease: "easeInOut" }}
        className="relative w-56 h-56 md:w-80 md:h-80 rounded-[2rem] overflow-hidden border border-white/10 shadow-2xl"
      >
        ...
        <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }}>
          ...
          <div className="max-w-4xl space-y-6 text-gray-400 leading-relaxed text-lg font-light">
            <motion.p variants={springUp}>
              I am a final-year B.Tech IT student (Batch of 2026) and a data-driven problem solver with deep expertise in Computer Vision, predictive modeling, and business intelligence.
            </motion.p>
            <motion.p variants={springUp}>
              As the Lead Data Scientist at Codeniet, I architect robust data models and ML pipelines. Alongside my startup, I operate as a freelance BI Analyst (@senthamil_py) optimizing workflows for international clients.
            </motion.p>
            <motion.p variants={springUp}>
              A major focus of my current work is <span className="text-white font-medium">Explainable AI (XAI)</span> and utilizing advanced tools like Power BI and SQL (CTEs, Window Functions) to drive business-wide analytics.
            </motion.p>
          </div>
        </motion.div>
      </section>
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            ...
            <motion.div variants={springUp} whileHover={{ y: -5 }} className="transition-transform h-full">
              <SpotlightCard className="h-full">
                ...
                <div className="flex flex-wrap gap-2">
                  {['Advanced SQL', 'Power BI', 'Data Warehousing', 'ETL Pipelines', 'Data Modeling', 'KPI Dashboards'].map((skill) => (
                    <span key={skill} className="px-4 py-2 bg-white/5 border border-white/10 rounded-full text-sm text-gray-300">{skill}</span>
                  ))}
                </div>
              </SpotlightCard>
            ...
            <motion.div variants={springUp} whileHover={{ y: -5 }} className="transition-transform h-full">
              <SpotlightCard className="h-full">
                ...
                <div className="flex flex-wrap gap-2">
                  {['Cursor AI', 'GitHub Copilot', 'PyCharm', 'Git & GitHub', 'Excel'].map((skill) => (
                    <span key={skill} className="px-4 py-2 bg-white/5 border border-white/10 rounded-full text-sm text-gray-300">{skill}</span>
                  ))}
                </div>
              </SpotlightCard>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer}>
          ...
          <div className="space-y-16 max-w-4xl">
            <motion.div variants={springUp} whileHover={{ x: 10 }} className="group border-l border-white/20 pl-8 relative transition-all duration-300">
              ...
              <h4 className="text-2xl font-medium text-white">Lead Data Scientist</h4>
              <div className="text-gray-500 mb-4 mt-1">
                <a href="https://codeniet.me" target="_blank" rel="noopener noreferrer" className="text-gray-300 hover:text-white underline decoration-white/30 underline-offset-4 transition-colors">Codeniet Startup</a> • Jan 2026 - Present
              </div>
              <p className="text-gray-400 font-light leading-relaxed">Spearheaded the AI and data analytics strategy, architecting robust data models and ML pipelines. Engineered scalable data warehousing frameworks to support business-wide analytics.</p>
            </motion.div>

            <motion.div variants={springUp} whileHover={{ x: 10 }} className="group border-l border-white/20 pl-8 relative transition-all duration-300">
              <div className="absolute w-3 h-3 bg-white/40 group-hover:bg-white rounded-full -left-[6.5px] top-2 transition-all duration-300 group-hover:shadow-[0_0_15px_rgba(255,255,255,0.8)]" />
              <h4 className="text-2xl font-medium text-white">Freelance BI Analyst & Data Scientist</h4>
              <div className="text-gray-500 mb-4 mt-1">@senthamil_py • Jan 2025 - Present</div>
              <p className="text-gray-400 font-light leading-relaxed">Delivered custom machine learning models and end-to-end data pipelines for international clients. Optimized database operations utilizing advanced SQL queries (CTEs, window functions).</p>
            </motion.div>
          </div>
        </motion.div>
      </section>
            ...
            <motion.div variants={springUp} whileHover={{ y: -10 }} className="transition-transform h-full">
              <SpotlightCard className="p-10 h-full">
                <div className="flex-1">
                  ...
                  <p className="text-gray-400 font-light leading-relaxed mb-8 text-lg">Designed and developed a complete, responsive web portal for an ITI college to streamline their digital presence and manage institutional data.</p>
                  <div className="flex flex-wrap gap-2 mb-8">
                    {['Web Development', 'UI/UX', 'Database', 'Project Management'].map((tech) => (
                      <span key={tech} className="text-xs px-4 py-2 rounded-full border border-white/10 text-gray-300">{tech}</span>
                    ))}
                  </div>
                </div>
      ...
      <section id="contact" className="py-40 px-6 max-w-4xl mx-auto text-center border-t border-white/5 relative z-20 bg-[#0a0a0a]">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={springUp}>
          ...
          <p className="text-gray-400 mb-12 text-lg font-light max-w-xl mx-auto">Actively seeking entry-level Data Scientist or Data Analyst roles.</p>
